...
Host 调用现有裁决器时使用固定、显式的参数映射：`hostAction -> playerAction`，`guestAction -> aiAction`，同时传入 `Host.player` 和 `Host.enemy`。Guest 发起攻击时不交换两个 `TopEntity`，而是让该动作进入现有裁决器的 `aiAction` 分支；裁决完成后再把 Host 视角结果转换为基于 `host/guest` 的协议 DTO。这样可以复用现有双向攻击分支，并避免主体反转或伤害目标错误。
...
直接覆盖是本 MVP 的有意约束。若双窗口前台测试仍出现不可接受的 20Hz 位移跳变，后续迭代可在协议不变的情况下增加仅作用于渲染坐标的目标点跟随；首版不在物理 `position` 上使用 `lerp`，避免本地碰撞计算持续读取滞后的远端坐标。
...
房间匹配完成后立即启动 10 秒准备计时器。若两端未在期限内全部发送 `CLIENT_READY`，服务器向仍在线的成员发送 `ERROR`（`READY_TIMEOUT`），清理该房间并让客户端返回主菜单；收到双方 Ready 后立即清除该计时器。
...
服务器每 2 秒向连接发送 WebSocket 协议级 Ping。浏览器会自动回复 Pong；连接连续 5 秒未确认存活时，服务器调用 `terminate()`，并沿用普通断线清理流程通知房间中的另一方。正常关闭标签页仍依赖即时 `close` 事件，无需等待心跳超时。
...
- 双标签页验收时应把页面拆成两个独立窗口并排显示，确保两端均处于可见状态。首版不引入 Web Worker；Worker 只能改善状态发送定时，不能解决后台页面的渲染暂停和玩家动作无法及时提交。
- Host 断线立即结束比赛，不迁移 Host。
- `startAt` 使用本机绝对时间仅适用于当前双标签页 MVP；跨设备阶段必须增加时钟偏移估计或改用服务器下发的相对延迟。
...
- 验证关闭任一连接后房间和计时器被清理。
- 验证任一客户端未发送 `CLIENT_READY` 时，房间在约 10 秒后以 `READY_TIMEOUT` 解散。

### 10.2 浏览器端到端测试

- 用 Playwright 打开两个页面，点击“真人联机”，确认自动匹配、不同角色和统一开战。
- 手动体验时将两个页面拆为并排可见窗口，记录后台节流与前台 20Hz 表现差异。
...
- 模拟无 `close` 帧的失活连接，确认服务器在约 5 秒内通过 Ping/Pong 检测并清理房间。
- 回归单机快速战斗、锦标赛和生存模式。
