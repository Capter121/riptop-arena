...
- 回冲速度强度必须不低于触发前原速度的 2 到 3 倍
- 生效帧需额外将实体朝场地中心微调约 `0.05` 单位
- 目的是避免自研积分器在下一帧再次判定为“仍处于触墙状态”，从而进入连续回冲死循环
...
`lockStability` 的推荐扣减基准为：

`relativeVelocity.length() * attacker.damageMultiplier * target.lockStabilityLossMultiplier`

其中：

- `relativeVelocity` 取碰撞瞬间双方相对速度
- `attacker.damageMultiplier` 体现强袭方针和技能强化带来的破锁压制
- `target.lockStabilityLossMultiplier` 体现目标在当前状态下对锁扣损伤的承受倍率

若实现时已有可靠的法线投影速度，可优先使用碰撞法线方向上的相对速度分量，以获得更稳定的命中手感和更符合对撞直觉的爆裂推进效果
...
该逻辑必须保证“单次消费”，且在消费后不会因边界残留再次触发。

### 9.10 一次性标记清理

所有一次性 Flag 的清理必须集中处理，不允许分散在多个系统中手工重置。

建议新增统一入口，例如：

- `clearTransientFlags(top)`
...
禁止在物理、技能、特效系统中各自零散清理，以避免状态悬空或多处竞态。
...
- 被护盾挡住时不进入爆裂
- 视觉仍保留轻量命中反馈
- 命中护盾时应触发专用电磁波纹或辉光震荡反馈，而不是常规金属火花
...
- 生效后立刻清除状态
- 不可无限连锁
- 生效帧会将实体轻微推回场内，避免下一帧重复判墙

### 12.4 方针与技能叠层验证

- `ASSAULT + INFINITE_STORM` 叠加时数值符合预期
- 技能结束后属性完全回到方针基线
- `lockStability` 扣减会随攻击倍率变化而同步放大
