...
async function sampleCanvas(page) {
  const visibleCanvases = page.locator('canvas').filter({ visible: true });
  const canvasIndex = await visibleCanvases.evaluateAll((canvases) => {
    return canvases
      .map((canvas, index) => {
  ...
  const locator = visibleCanvases.nth(canvasIndex);
  ...
  const ok = alphaPixels > 256 && (max - min > 8 || colors.size > 3);
  return {
    ok,
